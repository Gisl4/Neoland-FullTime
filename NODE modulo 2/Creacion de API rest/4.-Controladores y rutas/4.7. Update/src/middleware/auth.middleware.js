...
const isAuth = async (req, res, next) => {
  ...
  try {
    const decoded = verifyToken(token, process.env.JWT_SECRET); // el token se codifica con el verifyToken

    /// solo se crea req.user cuando es un endpoint authenticado ---> tiene como middleware el auth
    req.user = await User.findById(decoded.id); // con el "decoded.id" se saca el usuario y se guarda en el user---Antes de ser modificado
    next();
  } catch (error) {
    return next(error);
  }
