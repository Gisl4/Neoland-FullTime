...
const checkNewUser = async (req, res, next) => {
  ...
  */
  ...
  try {
    ...
    // El userExists hace referencia a lo que se encuentra arriba con el findOne
    ...
    if (!userExists) {
      ...
      if (confirmationCode === userExists.confirmationCode) {
        try {
          await userExists.updateOne({ check: true }); //Se actualiza con el updateOne
...
const changePassword = async (req, res, next) => {
   ...
   */
  //genera la contraseña y compruba que existe el usuario
  try {
    ...
    const userDb = await User.findOne({ email }); // se hace un findOne del usuario
    if (userDb) {
      /// si el usurio existe en la Db hacemos el redirect
      const PORT = process.env.PORT; //*Si el puerto cambia se puede cambiar dinamicamente
      return res.redirect(
        307, //se llama a un controlador que se encarga de enviar la contraseña
        `http://localhost:${PORT}/api/v1/users/sendPassword/${userDb._id}`
      );
    } else {
      return res.status(404).json("User no register"); // en caso de que no exista el usuario se dice "usuario no registrado"
    }
  } catch (error) {
    return next(error); //se capturan los errores
  }
};
...
const sendPassword = async (req, res, next) => {
  try {
    ...
    //recibimos un id
    const { id } = req.params;
    const userDb = await User.findById(id); //Buscar el usuario en la base de datos
    const email = process.env.EMAIL; //Trae el email
    const password = process.env.PASSWORD; //Traer el password
    //Crear el transporte donde nos autenticamos en gmail
    const transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: email,
    ...
    let passwordSecure = randomPassword(); //se genera una contraseña random en "util"
    //se genera el email
    console.log(passwordSecure); //se saca el console.log para ve que todo esta bien al exportarlo
    const mailOptions = {
      //se realiza el email
      from: email,
      to: userDb.email,
      ...
      //se realiza la carta donde se pone usuario, el nombre, el nuevo codigo, la contraseña segura y el texto que lleva la carta
      //*No poner en el texto password porque se borran del correo
    };
    //sendMail se envia la carta y la funcion asincrona
    transporter.sendMail(mailOptions, async function (error, info) {
      if (error) {
        ...
        const newPasswordBcrypt = bcrypt.hashSync(passwordSecure, 10); //hashSync se encripta la contraseña y las vueltas para hasheala

        //Siempre que se tiene una actualizacion o un borrado se tiene que meter un "try catch"
        // Un "try catch" por cada actualizacion y todo siempre va anidado
        try {
           ...
           */
          //*La qery findByIdAndUpdate apunta a un elemento en concreto, NO apunta al modelo
          await User.findByIdAndUpdate(id, { password: newPasswordBcrypt });
          //El metodo findByIndAnUpdate que hace buscar por id y actualizar
          //se le da la orden que se encuentre por el id que es el que recibi por el params para enviarle un objeto en donde va a colkocar la nueva contraseña encriptada

          //Despues de tenerlo actualizado se realiza un test con el cual se comprueba qu ela contraseña que esta encriptada si se ha cambiado
          //!------------------ test --------------------------------------------
          ...
          if (bcrypt.compareSync(passwordSecure, userUpdatePassword.password)) {
            ...
            return res.status(200).json({
              //si es igual se lanza un 200
              updateUser: true, //usuario actulizado
              sendPassword: true, //contraseña enviada
            });
            //en caso de que no pase lo anterior
          } else {
            ...
            return res.status(404).json({
              updateUser: false,
              sendPassword: true,
            }); //volver a pedir la contraseña que no se ha actualizado
          }
        } catch (error) {
          return res.status(404).json(error.message); //cuando es respuesta se envia "error.message"
        }
      }
    });
  } catch (error) {
    return next(error);
    //retornar next con error 500
    //cuando es next solo meter error
  }
};

//Comprobar en medio de la ruta si realmente ser tiene token o no se tiene token...realizando middleware de autentificacion para ver que el usuario realmente lo esta
