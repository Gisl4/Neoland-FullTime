...
//! ------------------CAMBIO DE CONTRASEÑA CUANDO ESTAS LOGADO---------------
...
const modifyPassword = async (req, res, next) => {
  ...
  try {
    ...
    // Recibimos la password antigua y la neva por el body
    const { _id } = req.user;
    // Desestructuro el id de la req.user

    /** comparamos la contrasela vieja sin encriptar y la encriptada */
    //* La vieja es "password" con "req.user.password", que esta en la middleware
    if (bcrypt.compareSync(password, req.user.password)) {
      /** tenemos que encriptar, con 10 vueltas la contraseña para poder guardarla en el back mongo db */
      const newPasswordHashed = bcrypt.hashSync(newPassword, 10);

      /** vamos a actualizar la contraseña en mongo db */
      //siempre que se tiene actualizacion nuevo hay un Try catch
      try {
        await User.findByIdAndUpdate(_id, { password: newPasswordHashed });
        //Al modelo le decimos "findByIdAndUpdate" que el usuario no encuentre la contraseña
...
const update = async (req, res, next) => {
  // capturamos la imagen nueva subida a cloudinary
  let catchImg = req.file?.path; //con el "req.file" se captura la imagen... se encuentra la URL

  try {
    // actualizamos los elementos unique del modelo
    await User.syncIndexes(); //cada que se tenga modificacion o actualizacion del modelo se pone la sincronizacion de los indexes
    //*Indexes - son elementos unicos

    // instanciamos un nuevo objeto del modelo de user con el req.body
    const patchUser = new User(req.body); //*Crear un usuario que este parcheado con una nueva instancia del modelo
    ...
    try {
       ...
       */
      await User.findByIdAndUpdate(req.user._id, patchUser); //"findByIdAndUpdate" apunta al modelo para dar el id del usurio y buscar
      //"patchUser" - se pasa para cambiar las claves
      ...
      // es lo que viene inicialmente del token que pasa pòr el middleware de autentificacion
      if (req.file) deleteImgCloudinary(req.user.image);

      // ++++++++++++++++++++++ TEST RUNTIME++++++tiempo de ejecucion de la aplicacion++++++++
      ...
      updateKeys.forEach((item) => {
        ...
        //updateKeys es un array de llaves que se todos los array se recorren con el forEach
        //*Para acceder a la clave de un objeto siempre se hace con el nombre del objeto "updateUser"
        //si es igual es de que si se ha actualizado
        if (updateUser[item] === req.body[item]) {
          ...
          if (updateUser[item] != req.user[item]) {
            ...
            testUpdate.push({
              [item]: "sameOldInfo", //agrega un objeto que hace que la clave es true o same o la imagen para meterla en la relacion "testUpdate"
            });
          }
        } else {
          testUpdate.push({
            [item]: false,
          }); //*con el corchete toma directamente el valor de la variable
        }
      });
      ...
      if (req.file) {
         ...
         */
        //*updateUser.image se tiene la url del usuario que se actualizo... se coje la imagen para ver que coincida con la de cloudinary
      ...
      if (req.file) deleteImgCloudinary(catchImg); //cuando se tiene error si hay una imagen se borra
      return res.status(404).json(error.message);
    }
  } catch (error) {
    if (req.file) deleteImgCloudinary(catchImg); // se borra la imagen
    return next(error); //se lnza un next con un error que se guarda en el log
  }
};

//? ---------------------------------DELETE--------------------------------------
// Solo se borra el modelo del usuario
const deleteUser = async (req, res, next) => {
  try {
    ...
    if (await User.findById(_id)) {
      // si el usuario
      return res.status(404).json("not deleted"); // si lo ha encontrado no lo ha borrado
    } else {
      /**
...
const byId = async (req, res, next) => {
  try {
    ...
    if (userById) {
      return res.status(200).json(userById); //retornar una respuesta
      //en caso de que lo anterior sea null
    } else {
      return res.status(404).json("usuario no encontrado"); //se devuelve un 404
    }
  } catch (error) {
    return next(error);
  }
...
const getAll = async (req, res, next) => {
  try {
    ...
    // Apuntamos al modelo y se hace un find
    if (getAllUser.length === 0) {
      return res.status(404).json("no encontrados");
    } else return res.status(200).json({ data: getAllUser }); // en caso contrario de tener elementes, se envian los elementos
  } catch (error) {
    return next(error);
  }
};
...
const byName = async (req, res, next) => {
  try {
    const getNameUser = await User.findOne({ name: req.params.name }); // Por la qery recibo por el param un name
    //al objeto creado le digo que me busque algo que tenga "req.params.name" ese nombre
    if (getNameUser) {
...
const byGender = async (req, res, next) => {
  try {
    ...
    }); // busque en el objeto creado algo que tenga gender y name
    if (getGenderUser) {
