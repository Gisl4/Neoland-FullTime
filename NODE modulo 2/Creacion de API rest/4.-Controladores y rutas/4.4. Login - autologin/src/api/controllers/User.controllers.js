...
const login = async (req, res, next) => {
  ...
  try {
    //se recibe el email y password
    const { email, password } = req.body;
    const userDB = await User.findOne({ email });
    //buscar el usuario con un findOne del email
    if (userDB) {
      // bcrypt=compara dos contraseñar una sin encryptar y otra encriptada
      //si existe usuario realizo una cosa, se compara una contraseña no encriptada con una encriptada
      if (bcrypt.compareSync(password, userDB.password)) {
        const token = generateToken(userDB._id, email);
        //generar el token para guardarlo en la constante"gerateToken" importada y se le pasa el id y email
        return res.status(200).json({
          user: userDB,
          token,
        }); //lazar una respuesta 200 donde se le dice cual es el usuario y mandarlo al token
      } else {
        //si la contraseña no es correcta poner que no conicide
        return res.status(404).json("password dont match");
      }
    } else {
      //si no existe usuario poner error
      return res.status(404).json("User no register");
    }
  } catch (error) {
    return next(error.message);
  } // error general lanzando un mensaje
};
...
//Los controladores son consumidos por las rutas
